...
public class BaseModelGenerator {
	...
	protected String classDefineTemplate =
		"/**%n" +
		" * BaseModel, Generated by Robot on %s, do not modify this file.%n" +
		" */%n" +
	...
	protected void genPackage(TableMeta tableMeta, StringBuilder ret) {
		ret.append(String.format(packageTemplate, baseModelPackageName, tableMeta.modelName.toLowerCase().replaceAll("_", "")));
	}
	...
	protected void genClassDefine(TableMeta tableMeta, StringBuilder ret) {
		ret.append(String.format(classDefineTemplate, Constants.DATE_TIME, tableMeta.baseModelName, tableMeta.baseModelName));
	}
	
	protected void genSetMethodName(ColumnMeta columnMeta, StringBuilder ret) {
		String setterMethodName = StrKit.firstCharToUpperCase(columnMeta.attrName);
		...
		String setter = String.format(setterTemplate, "set" + setterMethodName, columnMeta.javaType, argName, columnMeta.name, argName);
		ret.append(setter);
	}
	
	protected void genGetMethodName(ColumnMeta columnMeta, StringBuilder ret) {
		String getterMethodName = StrKit.firstCharToUpperCase(columnMeta.attrName);
		String getter = String.format(getterTemplate, columnMeta.javaType, "get" + getterMethodName, columnMeta.name);
		ret.append(getter);
	}
	...
	protected void wirtToFile(TableMeta tableMeta) throws IOException {
		File dir = new File(baseModelOutputDir + File.separator + tableMeta.modelName.toLowerCase().replaceAll("_", "") );
		if (!dir.exists())
			dir.mkdirs();
		
		String target = baseModelOutputDir + File.separator + tableMeta.modelName.toLowerCase().replaceAll("_", "") + File.separator + tableMeta.baseModelName + ".java";
